 ...
 * a ModifyTable node referencing a jdbc2_fdw foreign table.  We store:
 *
...
/*
 * Execution state of a foreign scan using jdbc2_fdw.
 */
 ...
 */
Datum
jdbc2_fdw_handler(PG_FUNCTION_ARGS)
{
                        ...
                        Oid foreigntableid)
{
    ...
    /*
     * We're not using remote estimates, so we stop here.
     */
    return;
}
...
//ereport(ERROR, (errmsg("\"fdw_private = %s\"\n",nodeToString(fdw_private))));
    /*
...
jdbcBeginForeignScan(ForeignScanState *node, int eflags)
{
    ...
    // table->options contains the query and/or table specified for the foreign table
    table = GetForeignTable(RelationGetRelid(fsstate->rel));
    // server-options contains drivername, url, querytimeout, jarfile and maxheapsize
...
/*
 * Acquire a random sample of rows from foreign table managed by jdbc2_fdw.
 *
